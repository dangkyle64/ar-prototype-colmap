...
import { handleOutputErrors } from '../../../services/colmapServicesHelpers/zipPLYFileHelpers/handleOutputErrors';
...
describe('handleOutputErrors', () => {
    ...
    it('should handle non-object errors like strings', () => {
        const reject = vi.fn();
        const errorHandler = handleOutputErrors(reject);
        ...
        expect(reject).toHaveBeenCalledWith(
            new Error('File write error: Unknown error')
        );
    });

    it('should throw if error is null', () => {
        const reject = vi.fn();
        const errorHandler = handleOutputErrors(reject);
    
        expect(() => errorHandler(null)).toThrow('No error provided');
    });
    
    it('should throw if reject is a non-function value', () => {
        const reject = 'notAFunction';
        const errorHandler = handleOutputErrors(reject);
    
        expect(() => errorHandler(new Error('fail')))
            .toThrow('reject is not a function');
    });
});